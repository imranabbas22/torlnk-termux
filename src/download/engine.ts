...
export class TorrentEngine {
  ...
  private ensureClient(): WebTorrent {
    if (!this.client) {
      ...
      // On Android/Termux, similar UDP port conflicts are common, and
      // the native WebRTC module rarely compiles, so disable NAT-PMP
      // there too.
      const opts = process.platform === "darwin" || process.platform === "android" ? { natPmp: false } : {};
